...
Download_brook(){
	...
	cd ${file}
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -N "https://github.com/keybook1/keybook/releases/download${brook_new_ver}/brook"
	else
		wget --no-check-certificate -N "https://github.com/keybook1/keybook/releases/download${brook_new_ver}/brook_linux_386"
...
Service_brook(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/keybook1/keybook/master/brook-pf_centos -O /etc/init.d/brook-pf; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/keybook1/keybook/master/brook-pf_debian -O /etc/init.d/brook-pf; then
			echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/brook-pf
